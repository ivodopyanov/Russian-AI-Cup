...
public class WeightFunctions
{
	...
	{
		int distance = DistanceCalculator.INSTANCE.getDistance(medic.getX(),
		        medic.getY(), teammate.getX(), teammate.getY(), world);
		return Constants.GO_TO_WOUNDED_TEAMMATE
		        / (teammate.getHitpoints() / 20 + 1) * (distance / 5 + 1);
	}
