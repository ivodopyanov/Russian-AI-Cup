...
public class CoverCalculator
{
	...
	public List<Direction> findCover(Trooper self, List<Trooper> enemies, World world)
	{
		...
		return Helper.INSTANCE.getDirectionForPath(self.getX(), self.getY(),
		        nearestCover.getX(), nearestCover.getY(), world);
	}
	...
	private Cell findCoverCell(Trooper self, List<Trooper> enemies, World world)
	{
		...
		while (!cellsToVisit.isEmpty())
		{
			...
			cellsToVisit.addAll(findCellsToVisit(cellToVisit, world,
			        visitedCells));
		}
	...
	private boolean isVisibleForEnemies(Cell cell, Trooper self, List<Trooper> enemies, World world)
	{
		...
		{
			if (world.isVisible(enemy.getShootingRange(), cell.getX(),
			        cell.getY(), self.getStance(), enemy.getX(), enemy.getY(),
			        enemy.getStance()))
			{
				return true;
			}
