...
public class ShootEnemy extends MoveEvaluatorImpl
{
	...
	{
		List<Trooper> reachableEnemies = Helper.INSTANCE.findVisibleEnemies(
		        self, world);
		...
		Collection<Trooper> teammatesNearby = Collections2.filter(
		        Helper.INSTANCE.findSquad(world),
		        new Helper.TrooperDistanceFilter(new Cell(self.getX(), self
		                .getY()), 5, world));
		boolean shouldRun = decideShootOrRun(reachableEnemies, teammatesNearby,
		        self);
		if (shouldRun)
		{
	...
	{
		int maxPossibleDamage = calcMaxPossibleDamage(reachableEnemies)
		        / teammatesNearby.size() + 1;
		return maxPossibleDamage >= self.getHitpoints()
		        || Constants.DEADLY_WOUNDED_HP >= self.getHitpoints();
	}
	
	private void findCover(Trooper self, List<Trooper> enemies, World world)
	{
		
		for (Direction direction : CoverCalculator.INSTANCE.findCover(self,
		        enemies, world))
		{
	...
	private void findTarget(Trooper self, List<Trooper> reachableEnemies, World world)
	{
		...
		MoveEvaluation result = MoveEvaluation.shoot(reachableEnemies.get(0)
		        .getX(), reachableEnemies.get(0).getY());
